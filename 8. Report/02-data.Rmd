...
```{r data-flow, fig.cap="Flow of data from sources to dataset", echo=FALSE,fig.height=200}
...
p$layers[[3]]$aes_params$size <- 0.65
...
`r if(eval_book){"\\newpage"}`
`r if(eval_book){"\\begin{landscape}"}`
...
p <- dataset |>
  ...
  ggplot(aes(x=Age_Baby_Boomers,y=COAL,text=DivisionNm)) +

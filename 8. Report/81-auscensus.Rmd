`r if(eval_book)'\\newpage'`
# {auscensus} Vignette {#auscensus-vignette}
...
tables_summary <- census_tables |>
                  ...
                  pivot_longer(cols=-Number, names_to="Year",values_to = "Value") |>
                  filter(!is.na(Value)) |>
                  select(-Value)
...
for(i in 1:nrow(tables_summary)){
  ...
  attr_i <- list_census_attributes(table_i) |> 
             pivot_longer(cols=-c(Table,Attribute),
...
codes <- c("SA1_7DIGITCODE_2011","SA1_7DIGITCODE_2016","SA1_CODE_2021")
ceds  <- c("CED_NAME_2011","CED_NAME_2016","CED_NAME_2021")

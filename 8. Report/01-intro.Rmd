...
`r if(eval_book) '\\allsectionsfont{\\raggedright}'`
...
- Candidates are sorted by their respective number of votes. If a candidate obtains an absolute majority (i.e. one vote above half of the valid ballots), they are declared the winner.
...
A graphical example of preference flows for the Division of Cooper is presented in figure \@ref(fig:flow-indi).

```{r flow-indi,fig.cap="Preference flow for Cooper 2022"}
auspol::house_preference_flow_plot("Cooper",2022) +labs(title="Preference flow for Cooper, 2022 Election")
```
...
- Is there a demographic divide between the inner city and suburbia? ([@doesthe],[@wehave])
...
- Was the so-called “Teal Wave” supported by discontent moderate Liberal voters? ([@wahlquist_teal_2022])
- Are Millenials not becoming more conservative as they age? ([@votingbygen],[@noauthor_millennials_nodate])
- Is culturally diverse voting relevant? ([@jakubowicz])

...
consolidated <-  tbl(source_db, "primary_vote_relative") |>
  ...
  select(Year,DivisionNm,PartyAb,StateAb,Percentage_Diff_State) |>
  pivot_wider(names_from = PartyAb,values_from=Percentage_Diff_State)  |>
...
consolidated |>
  collect() |>
  write_csv(here("4. Data","consolidated_state.csv"))

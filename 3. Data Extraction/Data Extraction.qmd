...
write_csv(household_tenure_attributes,"household_tenure_levels.csv")
 ...
 for(i in 1:length(census_years)){
   ...
   value_i <- tbl(mydb,"household_tenure_granular") |> 
    ...
    value_i <- value_i  |> 
               auscensus::calculate_percentage(key_col = "Attribute",
                                                value_col = "Value",
                                                key_value = "household_tenure - Total",
                                                percentage_scale = 100)
...
household_tenure_attributes <- read_csv("household_tenure_edited.csv")
...
capital_cities <- aussiemaps::get_map(filters = list(LGA_NAME_2022=c("Melbourne",
                                                                     "Sydney",
                                                                     "Adelaide",
                                                                     "Brisbane",
                                                                     "Darwin",
                                                                     ...
                                                                     "Canberra",
                                                                     "Hobart")),
                                      year=2021,aggregation="LGA_NAME_2022")

capital_cities <- capital_cities |>
                  st_drop_geometry() |>
                  select(LGA_NAME_2022,STATE_NAME_2021) |>
                  bind_cols(as_tibble(capital_cities |> 
                         st_centroid() |> 
                         st_coordinates())) |>
...
data_i  <- get_silodata(latitude = capital_cities[i,]$latitude,
             longitude = capital_cities[i,]$longitude,
             email = "carlos.yanez.s@gmail.com",
             START = "20050101",
             FINISH = "20220731") |>
          mutate(CapitalCity = capital_cities[i,]$LGA_NAME_2022,
                 State       = capital_cities[i,]$STATE_NAME_2021)

silodata <- bind_rows(silodata,data_i)
...
dbWriteTable(mydb, "silodata", silodata,overwrite=TRUE)

...
for(year in unique(electorates_states$Year)){
  ...
  prior_i  <- tbl(original_db,"prior_res_full") |>
                       collect() |>
  ...
  for(year in years){
     ...
     result_i_total <- result_i |> 
       filter(str_detect(Attribute,"[Tt]otal")) |>
                select(-Census_Code,-Attribute) |>
                rename("Total"="Value")  
     
      result_i<-    result_i |>
              filter(str_detect(Attribute,"[Tt]otal",TRUE)) |>
              left_join(result_i_total,by=c("Unit","Year")) |>
               filter(Value!=0)                             |>
               filter(Total!=0)                             |>
               mutate(Percentage=100*Value/Total)           |>
              select(-Value,-Total)                          |>
            left_join(correspondence_i,by=c("Unit","Year")) |>

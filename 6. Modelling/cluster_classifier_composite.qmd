...
### Remove cluster 1, test criteria with criteria from prior iterations

```{r}
dataset_1 <- dataset |> filter(cluster==1)

dataset_1 |>
  mutate(is_1 = ((Metro==0) & !(Language_English_Only <80))) |>
  summarise(is_1 = mean(is_1))
...
```{r}
set.seed(1234)
split <- initial_split(dataset_rest,prop=0.8)
...
saveRDS(extract_fit_parsnip(rpart_model),path(clustering_folder,"clustering_rpart_combined.rds"))
saveRDS(extract_fit_parsnip(ranger_model),path(clustering_folder,"clustering_ranger_combined.rds"))
...
```{r}
dataset_rest_orig <- dataset_rest
```
...
set.seed(1234)
dataset_rest <- dataset_rest |>
   select(any_of(c("cluster",
                   "Household_Flat",
                #  "Household_Standalone", highly correlated to Household_Flat
                  "Household_Owned_with_a_mortgage",
                  "Relationship_Group_Household",
                  "Relationship_Married",
                  "Relationship_Single_Parent",

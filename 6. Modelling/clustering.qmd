...
```{r}
transform_pred <- MFA(train_clustering |>                        select(-any_of(c("StateAb","Metro_Area","Year","Metro")),                               -any_of(parties)) |>                        remove_rownames() |>                        column_to_rownames(var="Division"),                        group=groups$n,                        ncp=10,                        type=c(rep("c",8),"n"),                        name.group =groups$group,                        graph=FALSE)
```
...
```{r}
transform_pred$eig |>    as_tibble() |>   mutate(Dimension=glue("Dim {row_number()}"), .before=1) |>   gt()
```

...
dataset <- dataset_orig |>
           select(any_of(c("cluster","Language_English_Only","Metro")),
...
```{r}
dataset_metro <- dataset_orig |> 
                select(-Metro_Area,-StateAb) |>
                filter(Metro==1) |>
                mutate(cluster=fct_drop(cluster))

set.seed(1234)
split <- initial_split(dataset_metro,prop=0.8)
...
extract_workflow(ranger_model_metro) |>
  extract_fit_parsnip() |>
  vip()
...
```{r}
dataset_non_metro <- dataset_orig |> 
                select(-Metro_Area,-StateAb) |>
                filter(Metro==0) |>
                mutate(cluster=fct_drop(cluster))

set.seed(1234)
split <- initial_split(dataset_non_metro,prop=0.8)
...
extract_workflow(ranger_model_non_metro) |>
  extract_fit_parsnip() |>
  vip()
```

Metro accuracy becomes worse?

Non metro accuracy very good - but dataset is very imbalanced - Not going this way.
...
saveRDS(extract_fit_parsnip(ranger_model),
        path(clustering_folder,"clustering_ranger_redux.rds"))

saveRDS(extract_fit_parsnip(rpart_model),
        path(clustering_folder,"clustering_rpart_redux.rds"))
